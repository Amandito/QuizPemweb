...
@section('content')
<h1 class="font-bold text-2xl mb-5">Update Stock Product</h1>

<form action="{{ route('product.updateStock', $produk->id) }}" method="POST">
        @csrf
        @method('PUT')
  <div class="mb-6">
  ...
  <button type="submit" class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Update Stock</button>
  <a href="{{route('product.index')}}" class="ml-3 focus:outline-none text-white bg-red-400 hover:bg-red-500 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:focus:ring-red-900">Cancel</a>
</form>
@endsection